...
async function boardProposeBoardMember(functionToCall) {
  ...
  console.log(`Proposed new Board Member Name: ${board_member_name} with address: ${board_member_address}`);
  console.log(`New ${functionToCall} Proposal Description:\n  ${proposal_description}\n`);
  ...
  proposalsInfo["goerli"].push("ID: " + proposalId.toString(),
    ["Index: " + proposalIndex.toString(), "Function: " + BOARD_MEMBER_FUNC.toString(),
    "Caller Address: " + caller_address.toString(), "Board Member Name: " + board_member_name.toString(),
...
boardProposeBoardMember(BOARD_MEMBER_FUNC)
  .then(() => process.exit(0))
  .catch((error) => {
    console.error(error)
    process.exit(1)
