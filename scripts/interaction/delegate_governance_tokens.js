...
async function delegateGovernanceToken() {
  //const governanceTokenContract = await ethers.getContractAt("GovernanceToken");
  console.log(`Delegating Governance Tokens to: ${DELEGATED_ACCOUNT}`);

  const transactionResponse = await governanceTokenContract._mintToken();
  await transactionResponse.wait();
  console.log(`Delegated!`);

  console.log(`Checkpoints: ${await governanceTokenContract.numCheckpoints(DELEGATED_ACCOUNT)}`);
